    ...
    INestedTypeProducerNode
{
    ...
    {
        public TypePath Path { get; } = Path.Add<BackLinkNode>("BackLink<TSource>");
    ...
    private TypeSpec Build<TParent>(State<TParent> state, CancellationToken token)
    {
        ...
        if (!state.IsRoot)
        {
            ...
            for (var i = 2; i < state.Path.Count - 1; i++)
            {
                spec = spec.AddBases(
                    (
                        state.Path.First.Value +
                        state.Path.Slice(1).SemanticalProduct()
                    ).Select(x => x.ToString())
                );
            }
        }
            ...
            [
                ...
                ..hierarchy.Overloads.Select(path =>
                    new PropertySpec(
                        ...
                        $"{hierarchy.ActorInfo.Actor}.{path}.Hierarchy.BackLink<TSource>",
                        Expression: GetFriendlyName(x.Actor)
                    )
                )
            ])
